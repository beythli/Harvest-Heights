...
void cTileMap::render(SDL_Renderer* theRenderer, cTextureMgr* theTxtMgr, vector<LPCSTR> theTxt)
{
	SDL_Rect tilePosXY;
	tilePosXY = { mapStartXY.x,  mapStartXY.y, 0 , 0 };
	SDL_Rect theSpriteDims = { mapStartXY.x,  mapStartXY.y, 0 , 0 };
	...
	for (int row = 0; row < numRows; row++)
	{
		for (int column = 0; column < numCols; column++)
		{
			...
			SDL_Point theCentre = aTile.getSpriteCentre();
			theSpriteDims = aTile.getSpriteDimensions();
			// Render tile
			if (tileMap[row][column] == 3)
			{
				aTile.render(theRenderer, &theSpriteDims, &tilePosXY, this->getTileRotation(), &theCentre);
			}
			else
			{
				aTile.render(theRenderer, &theSpriteDims, &tilePosXY, aTile.getSpriteRotAngle(), &theCentre);
			}
